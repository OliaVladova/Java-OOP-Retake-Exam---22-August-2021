...
public class ControllerImpl implements Controller {
    ...
    public String exploreState(String stateName) {
        long retired = 0;
        List<Explorer> suitable = this.explorerRepository.getCollection().stream().filter(explorer -> explorer.getEnergy()>50).collect(Collectors.toList());
        if (suitable.isEmpty()){
            throw new IllegalArgumentException(ExceptionMessages.STATE_EXPLORERS_DOES_NOT_EXISTS);
        }
        Mission mission = new MissionImpl();
        mission.explore(this.stateRepository.byName(stateName),suitable);
        retired = suitable.stream().filter(explorer -> explorer.getEnergy()==0).count();
        this.exploredStates++;
